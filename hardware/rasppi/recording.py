...
class RecordingManager:
    ...
    def start(self, session_id: str, upload_url: str) -> dict:
        with self._lock:
            ...
            camera_names = self._cameras.fetch_local_names()
            ...
            for name in camera_names:
                if self._cameras.webrtc_mode:
                    self._cameras.get_or_start_ffmpeg(name)
                dest = session_dir / f"{_safe_name(name)}.mkv"
                try:
                    procs[name] = _start_recording_ffmpeg(
                        name, dest,
                        self._cameras.webrtc_mode,
                        self._cameras.rtsp_port,
                    )
                    files[name] = str(dest)
    ...
    def stop(self, expected_session: str = "") -> dict:
        ...
        for name, raw_path in sess["files"].items():
            ...
            if not fpath.exists() or fpath.stat().st_size <= 0:
                ...
                continue
            try:
                result           = _upload_recording(
                    sess["upload_url"], self._get_central_url(),
                    sess["id"], name, fpath,
                )
                result["camera"] = name
...
def _start_recording_ffmpeg(
    ...
    if webrtc_mode:
        cmd = [
            ...
            "-rtsp_transport", "tcp",
            "-i", f"rtsp://127.0.0.1:{rtsp_port}/{camera_name}",
        ...
        cmd = [
            ...
            "-i", f"/dev/{camera_name}",
            "-c:v", "copy", "-an", "-f", "matroska", str(dest),
        ]
